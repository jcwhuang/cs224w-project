...
class Player:
    ...
    def short_str(self): return '%s: %s' % (self.pid, self.name)

    def __str__(self):
        return 'Player{pid: %s, name: %s, team: %s, pos: %s, cost: %s}' % (self.pid, self.name, self.team, self.pos, self.cost)
...
class PlayerBulletin:
    def __init__(self, playerPath):
        ...
        """
        # Read players (JSON format)
        self.players = {}
        info = json.loads(open(playerPath).read())
        for playerInfo in info.values():
...
class PlayerRequest:
    ...
    def __str__(self):
        return 'PlayerRequest{%s %s}' % \
            (self.pids, self.weight)
...
class ProjectProfile:
    def __init__(self, bulletin, prefsPath):
        ...
        # Read preferences
        self.minValue = 100 * 10 # minimum units per quarter
        self.maxValue = 100 * 10 # maximum units per quarter
        # self.quarters = [] # (e.g., Aut2013)
        # self.taken = set()  # Courses that we've taken
        # self.requests = []
        request_dict = {pid: PlayerRequest([pid], 1) for pid in bulletin.players}
        for line in open(prefsPath):
            m = re.match('(.*)\\s*#.*', line)
            if m: line = m.group(1)
            line = line.strip()
            ...
            # Value
            m = re.match('minValue (.+)', line)
            if m:
                self.minValue = int(float(m.group(1)) * 10)
                continue
            m = re.match('maxValue (.+)', line)
            if m:
                self.maxValue = int(float(m.group(1)) * 10)
                continue
            ...
            if m:
                ...
                for i in range(0, len(args), 2):
                    ...
                    # elif args[i] == 'in':  # Take in a particular quarter
                    #     quarters = [self.ensure_quarter(q) for q in args[i+1].split(',')]
                    if args[i] == 'weight':  # How much is selecting this player worth
                        weight = float(args[i+1])
                    elif args[i].startswith('#'): # Comments
                        break
                    else:
                        raise Exception("Invalid arguments: %s" % args)
                request_dict[m.group(1)] = PlayerRequest(pids, weight)
                continue

            raise Exception("Invalid command: '%s'" % line)

        self.requests = request_dict.values()
        ...
        for req in self.requests:
            for pid in req.pids:
                if pid in self.taking:
                    raise Exception("Cannot request %s more than once" % cid)
            self.taking.update(req.pids)
    ...
    def print_info(self):
        print "Value: %d-%d" % (self.minValue, self.maxValue)
        # print "Quarter: %s" % self.quarters
        # print "Taken: %s" % self.taken
        print "Requests:"
        for req in self.requests: print '  %s' % req
...
def extract_project_solution(profile, assign):
    """
    Given an assignment returned from the CSP solver, reconstruct the plan. It
    is assume that (req, quarter) is used as the variable to indicate if a request
    is being assigned to a speific quarter, and (quarter, cid) is used as the variable
    ...
    for req in profile.requests:
            ...
            if pid not in assign:
                result.append((pid, None))
            else:
                result.append((pid, assign[pid]))
    return result
...
def print_project_solution(solution):
    """
    Print a schedule in a nice format based on a solution.
    ...
    if solution == None:
        ...
        print "Here's the best schedule:"
        print "Value\tPlayer"
        for player, value in solution:
            if value != None:
                print "  %s\t%s" % (value, player)
            else:
                pass  # print "  %s\t%s\t%s" % (quarter, 'None', course)
