...
class PredictPD():
	...
	def __init__(self):
		...
		# each player has its own set of weights
		self.weights = defaultdict(lambda: defaultdict(lambda: defaultdict(int)))
	...
	def evaluate(self, features, weight, teamName, p_key):
		score = self.computeScore(features, self.weights[teamName][p_key])
		# print "score %f vs. actual %f" % (float(score), float(weight))
		loss = self.computeLoss(features, self.weights[teamName][p_key], float(weight))
		# print "Loss: %f" % loss
		return (score, loss)
	...
	def updateWeights(self, features, weights, label, teamName, p_key):
		grad = self.computeGradientLoss(features, weights, label)
		for w in self.weights[teamName][p_key]:
			self.weights[teamName][p_key][w] -= self.stepSize * grad[w]
		...
		for i in xrange(num_iter):
			...
			for game in allGames:
				...
				for players in edgeFile:
					...
					p_key = p1 + "-" + p2
					score, loss = self.evaluate(features, weight, teamName, p_key)
 					self.updateWeights(features, self.weights[teamName][p_key], int(weight), teamName, p_key)
 					for w in self.weights[teamName][p_key]:
						print "weights[%s][%s][%s] = %f" % (teamName, p_key, w, float(self.weights[teamName][p_key][w]))
 					avgLoss += loss
					totalEx += 1
 				matchNum += 1
